...
final class AutomatorWP_Invite_Anyone_Integration {
    ...
    private function constants() {
        // Plugin version
        define( 'AUTOMATORWP_INVITE_ANYONE_VER', '1.0.1' );
    ...
    public function admin_notices() {
        ...
        if ( ! $this->meets_requirements() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php printf(
                        __( 'AutomatorWP - Invite Anyone requires %s and %s in order to work. Please install and activate them.', 'automatorwp-invite-anyone-integration' ),
                        '<a href="https://wordpress.org/plugins/automatorwp/" target="_blank">AutomatorWP</a>',
                        '<a href="https://wordpress.org/plugins/invite-anyone-integration/" target="_blank">Invite Anyone</a>'
                    ); ?>
                </p>
            </div>
        ...
        <?php elseif ( $this->pro_installed() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php echo __( 'You can uninstall AutomatorWP - Invite Anyone Integration because you already have the pro version installed and includes all the features of the free version.', 'automatorwp-invite-anyone-integration' ); ?>
                </p>
            </div>
    ...
    public function load_textdomain() {
        ...
        $lang_dir = AUTOMATORWP_INVITE_ANYONE_DIR . '/languages/';
        $lang_dir = apply_filters( 'automatorwp_invite_anyone_languages_directory', $lang_dir );

        // Traditional WordPress plugin locale filter
        $locale = apply_filters( 'plugin_locale', get_locale(), 'automatorwp-invite-anyone-integration' );
        $mofile = sprintf( '%1$s-%2$s.mo', 'automatorwp-invite-anyone-integration', $locale );

        // Setup paths to current locale file
        $mofile_local   = $lang_dir . $mofile;
        $mofile_global  = WP_LANG_DIR . '/automatorwp-invite-anyone-integration/' . $mofile;

        if( file_exists( $mofile_global ) ) {
            // Look in global /wp-content/languages/automatorwp-invite-anyone-integration/ folder
            load_textdomain( 'automatorwp-invite-anyone-integration', $mofile_global );
        } elseif( file_exists( $mofile_local ) ) {
            // Look in local /wp-content/plugins/automatorwp-invite-anyone-integration/languages/ folder
            load_textdomain( 'automatorwp-invite-anyone-integration', $mofile_local );
        } else {
            // Load the default language files
            load_plugin_textdomain( 'automatorwp-invite-anyone-integration', false, $lang_dir );
        }
